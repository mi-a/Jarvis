...
Deal = function(options) {
  options = options || {};
  var numZip = parseInt(options.location.zip);
  ...
  if (numZip >= 90001 && numZip <= 92899) {
    this.region = 'los-angeles';
  } else if (numZip >= 94002 && numZip <= 94998) {
    this.region = 'san-francisco';
  } else if (!options.location.zip) {
    this.region = 'all';
  }
...
Meteor.methods({
  ...
  getDealsByRegion: function(region) {
    Deals.find({ region: 'all' });
  },
